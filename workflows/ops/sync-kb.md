...
|----------|-------------|
| Sync summary report | Pull result, document change count, rebuild status, verification outcome |
| Updated embeddings | Rebuilt `knowledge-base/embeddings/` reflecting the latest documents |
| Verification result | Pass/fail from `verify-kb` confirming embedding count matches manifest |
