...
**Harness Ship** — Ship completed work with status aligned to proof.
...
3. Read `.ai-harness/commands/harness-ship.md` (full command contract).
4. Read `.ai-harness/prompt-templates/harness-ship.md`.
...
Execute the workflow defined in `.ai-harness/commands/harness-ship.md` for **this repository only**. Use existing local artifacts first; ask only when blocked.
