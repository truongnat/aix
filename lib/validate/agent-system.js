...
function assertAgentSystemLayer(baseDir, failures) {
  ...
  for (const provider of ["claude", "cursor", "codex", "gemini"]) {
    const template = path.join(baseDir, `rules/providers/${provider === "cursor" ? "cursor/ai-engineering-harness.mdc" : provider === "claude" ? "claude/CLAUDE.md" : provider === "codex" ? "codex/AGENTS.md" : "gemini/GEMINI.md"}`);
