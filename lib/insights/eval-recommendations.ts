import { buildInsights, type Summary } from "./index";

const GUARD_TO_TASKS: Record<string, string[]> = {
...
interface EvalRecommendationsResult {
  target: string;
  eventsPath: string;
  totalEvents: number;
...
function buildEvalRecommendations(targetRoot: string): EvalRecommendationsResult {
...
function formatEvalRecommendations(result: EvalRecommendationsResult): string {
  const lines = [
...
export { buildEvalRecommendations, formatEvalRecommendations };
export type { Recommendation, EvalRecommendationsResult };
