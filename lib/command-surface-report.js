...
function formatStatusCommandLines(targetRoot) {
  ...
  const surface = readInstalledCommandSurface(targetRoot);
...
function formatDoctorCommandLines(targetRoot, detectedRuntimes) {
  ...
  for (const rt of detectedRuntimes) {
    ...
    if (spec.status === "plugin-packaging" && rt === "codex") {
      ...
      lines.push(
        `WARN ${spec.provider}: ${spec.status}; ${formatProviderUseLine(rt)}`
      );
    } else if (spec.status === "native-command-files" || spec.status === "native-plugin") {
      lines.push(
        `PASS ${spec.provider}: ${spec.status} (${spec.actualInvocation || "see docs"})`
      );
      if (rt === "claude") {
      ...
      lines.push(`PASS ${spec.provider}: native command support verified (${spec.actualInvocation || "see docs"})`);
    }
