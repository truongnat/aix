...
function clearScreenLines(count) {
    ...
    node_readline_1.default.moveCursor(process.stdout, 0, -count);
...
async function selectMany(items, options) {
    ...
    let index = Math.max(0, items.findIndex((i) => i.implemented && (i.recommended || selected.has(i.id))));
    if (index < 0) {
...
async function selectManyFallback(selectable, title, minSelected) {
    ...
    const answer = await new Promise((resolve) => {
        rl.question(`Select (comma-separated numbers, default: ${selectable.find((i) => i.recommended)?.label || selectable[0].label}): `, resolve);
    });
    rl.close();
    ...
    const ids = indices
        .filter((i) => i >= 0 && i < selectable.length)
        .map((i) => selectable[i].id);
...
async function selectOneFallback(choices, options) {
    ...
    const rl = node_readline_1.default.createInterface({ input: process.stdin, output: process.stdout });
    const answer = await new Promise((resolve) => {
        rl.question(`Select [1-${choices.length}]${defaultIndex >= 0 ? ` (default ${defaultIndex + 1})` : ""}: `, resolve);
    });
    rl.close();
    const trimmed = answer.trim();
