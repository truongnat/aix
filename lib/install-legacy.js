...
function formatTargetDisplay(targetArg, resolvedTarget) {
    ...
    const relative = node_path_1.default.relative(root, resolvedTarget);
    if (relative === "") {
        return ".";
    }
...
function parseArgs(argv) {
    ...
    const resolvedTarget = node_path_1.default.resolve(options.target);
    return {
        ...options,
...
function listFiles(relativePath, depth = 0, maxDepth = 20, visitedInodes = new Set()) {
    const MAX_DEPTH = maxDepth;
    if (depth > MAX_DEPTH) {
        return [];
    }
    const sourcePath = node_path_1.default.join(root, relativePath);
...
function installHarness(options) {
    ...
    for (const relativePath of files) {
        const sourcePath = node_path_1.default.join(root, relativePath);
        const destinationPath = node_path_1.default.join(options.target, relativePath);
