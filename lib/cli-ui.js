...
const { intro, outro, cancel, confirm: clackConfirm, select, multiselect, spinner, note, isCancel, } = require("@clack/prompts");
...
function useInteractiveUi(options) {
    return (process.stdin.isTTY && process.stdout.isTTY && !options.yes && options.providers.length === 0);
}
...
function showInstallPlan(plan, { compact = false, providers = [] } = {}) {
    const commandBlock = providers.length > 0
        ? (0, runtime_command_catalog_1.formatCommandSupportForPlan)(providers)
        : (0, runtime_command_catalog_1.formatCommandSupportForPlan)(["cursor"]);
    const body = [
