...
2. \`.harness/STATE.md\`
3. Active session artifacts under \`.harness/sessions/\`
4. \`.ai-harness/prompt-templates/${spec.canonical}.md\` if present

Then follow the harness command contract at \`.ai-harness/commands/${spec.sourceCommand || `commands/${spec.canonical}.md`}\`.

If required preconditions are missing, return \`Blocked\` and stop.
...
function assertProviderRuleContent(relativePath, content, failures) {
  ...
  if (needsCore) {
    ...
    if (!content.includes("Stop.") && !content.includes("Stop and ask")) {
