...
const requiredFiles = [
  ...
  "docs/dispatch-prompt-templates.md",
...
function assertPromptTemplateContract(relativePath, content, failures) {
  ...
  if (
    /prompt-templates\/blocker-question\.md$/.test(relativePath) &&
    !/stop after asking|workflow is paused|no further phase was executed/i.test(content)
  ) {
...
function countCheckedContracts() {
  return (
    ...
    promptTemplateFiles.length * promptTemplateHeadings.length +
...
function validateHarnessRepository(baseDir = root) {
  ...
  for (const relativePath of promptTemplateFiles) {
    ...
    const content = readFile(baseDir, relativePath);
    assertPromptTemplateContract(relativePath, content, failures);
  }
