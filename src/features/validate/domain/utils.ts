...
function readFile(baseDir: string, relativePath: string): string {
  return fs.readFileSync(resolvePath(baseDir, relativePath), "utf8");
}

function extractMachineField(content: string, fieldName: string): string | null {
...
function hasSubstantiveSectionBody(
  sectionBody: string,
...
function hasConcreteFailureRule(sectionBody: string): boolean {
  if (!sectionBody || !sectionBody.trim()) {
    return false;
  }
  return sectionBody
...
function assertNonEmpty(baseDir: string, relativePath: string, failures: string[]): void {
  const fullPath = resolvePath(baseDir, relativePath);
  if (!fs.existsSync(fullPath)) {
    return;
  }
...
function parseNestedFrontmatterMap(content: string, key: string): Record<string, string> | null {
