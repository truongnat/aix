...
use super::{
    GitError, Result, GitProvider, PrInfo, PrState, CiState, MergeStrategy, MergeDecision,
    AutoMergePolicy, PrIntegration, CiIntegration,
};
...
impl AutoMerge {
    ...
    pub async fn merge(&self, pr: &PrInfo, strategy: MergeStrategy) -> Result<()> {
        ...
        if !decision.can_merge {
            return Err(GitError::MergeFailed(
                decision.reason.unwrap_or_else(|| "Cannot merge".to_string()),
            ));
        }
    ...
    async fn get_reviews(&self, pr_number: u64) -> Result<usize> {
        ...
        let review_count = pr.requested_reviewers.as_ref().map(|r| r.len()).unwrap_or(0);
    ...
    async fn merge_gitlab_mr(&self, _pr_number: u64, _strategy: MergeStrategy) -> Result<()> {
        ...
        Err(GitError::MergeFailed("GitLab not yet implemented".to_string()))
    }
}
