...
function getLanguage(file: string): keyof typeof PATTERNS | null {
...
function parseFile(filePath: string, baseDir: string): { nodes: GraphNode[]; edges: GraphEdge[] } {
  const nodes: GraphNode[] = [];
  const edges: GraphEdge[] = [];
  
  if (!existsSync(filePath)) return { nodes, edges };
...
function findContainingFunction(content: string, pos: number, nodes: GraphNode[]): string | null {
...
export async function buildGraph(
  baseDir: string,
  includePatterns: string[] = ['**/*.ts', '**/*.tsx', '**/*.js', '**/*.py', '**/*.go', '**/*.rs'],
  ignorePatterns: string[] = ['**/node_modules/**', '**/.git/**', '**/dist/**', '**/build/**', '**/.venv/**']
): Promise<CodeGraph> {
...
export function saveGraph(graph: CodeGraph, outputPath: string): void {
  writeFileSync(outputPath, JSON.stringify(graph, null, 2), 'utf8');
}

export function loadGraph(graphPath: string): CodeGraph {
  if (!existsSync(graphPath)) {
    throw new Error(`Graph file not found: ${graphPath}`);
  }
  return JSON.parse(readFileSync(graphPath, 'utf8')) as CodeGraph;
}

export function queryGraph(graph: CodeGraph, query: string): GraphNode[] {
...
export function getCallers(graph: CodeGraph, symbolName: string): GraphNode[] {
...
export function getCallees(graph: CodeGraph, symbolName: string): GraphNode[] {
  const targetIds = graph.nodes
    .filter(n => n.name === symbolName)
    .map(n => n.id);
...
export function impactAnalysis(graph: CodeGraph, filePath: string): GraphNode[] {
