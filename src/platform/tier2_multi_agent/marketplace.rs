...
use crate::platform::types::{AgentId, TrustTier, current_timestamp_ms};
...
impl AgentMarketplaceTrait for AgentMarketplace {
    fn register_agent(&mut self, agent: AgentProfile) -> Result<AgentId> {
        ...
        if self.agents.contains_key(&agent.agent_id) {
            return Err(PlatformError::InvalidInput(
                format!("Agent {} already registered", agent.agent_id),
            ));
        }
    ...
    fn search_agents(&self, criteria: &SearchCriteria) -> Result<Vec<AgentProfile>> {
            ...
            candidates.retain(|agent| {
                criteria.required_skills.iter().all(|required_skill| {
                    agent.specialization.contains(required_skill)
                        || agent.capabilities.iter().any(|cap| &cap.name == required_skill)
                })
            });
        }
        ...
        if let Some(min_proficiency) = criteria.min_proficiency {
            candidates.retain(|agent| {
                agent.capabilities.iter().any(|cap| cap.proficiency >= min_proficiency)
            });
        }
        ...
        if let Some(ref version_constraint) = criteria.version_constraint {
            candidates.retain(|agent| {
                matches_version_constraint(&agent.version, version_constraint)
            });
        }
    ...
    fn rate_agent(&mut self, agent_id: &AgentId, rating: Rating) -> Result<()> {
        ...
        let agent = self.agents.get_mut(agent_id).ok_or_else(|| {
            PlatformError::NotFound(format!("Agent {} not found", agent_id))
        })?;
...
mod tests {
    ...
    fn test_rate_agent() {
        ...
        marketplace.rate_agent(&"agent1".to_string(), rating).unwrap();
    ...
    fn test_update_performance() {
        ...
        marketplace.update_performance(&"agent1".to_string(), update).unwrap();
    ...
    fn test_version_constraint() {
        assert!(matches_version_constraint("1.0.0", &VersionConstraint::Exact("1.0.0".to_string())));
        assert!(!matches_version_constraint("1.0.1", &VersionConstraint::Exact("1.0.0".to_string())));
        
        assert!(matches_version_constraint("1.5.0", &VersionConstraint::MinVersion("1.0.0".to_string())));
        assert!(!matches_version_constraint("0.9.0", &VersionConstraint::MinVersion("1.0.0".to_string())));
