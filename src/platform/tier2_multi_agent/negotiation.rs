...
use crate::platform::types::{AgentId, current_timestamp_ms};
...
impl NegotiationProtocol {
    ...
    pub fn submit_position(
        ...
        let session = self.sessions.get_mut(session_id)
            .ok_or_else(|| PlatformError::NotFound(
                format!("Negotiation session not found: {}", session_id)
            ))?;
        
        // Check if session is still active
        if session.status != NegotiationStatus::Active {
            return Err(PlatformError::NegotiationError(
                format!("Cannot submit position to {} session", session.status)
            ));
        }
        ...
        if !session.topic.participants.contains(&agent_id.to_string()) {
            return Err(PlatformError::NegotiationError(
                format!("Agent {} is not a participant in this negotiation", agent_id)
            ));
        }
    ...
    pub fn resolve(&mut self, session_id: &str) -> Result<Resolution> {
        ...
        let (timed_out, all_submitted, positions_len, session_clone) = {
            ...
            if session.status != NegotiationStatus::Active {
                return Err(PlatformError::NegotiationError(
                    format!("Cannot resolve {} session", session.status)
                ));
            }
            
            let timed_out = current_timestamp_ms() > session.topic.deadline_ms;
            let all_submitted = session.topic.participants.iter()
        ...
        if !all_submitted && !timed_out {
            return Err(PlatformError::NegotiationError(
                "Not all participants have submitted positions and deadline has not passed".to_string()
            ));
        }
    ...
    fn resolve_by_consensus_static(session: &NegotiationSession) -> Result<Resolution> {
        ...
        let all_approve = positions.iter().all(|p| matches!(p.stance, Stance::Approve));
        ...
        let all_conditional = positions.iter().all(|p| matches!(p.stance, Stance::Conditional { .. }));
        ...
        if all_conditional {
            ...
            return Ok(Resolution {
                outcome: ResolutionOutcome::Consensus,
                consensus_level,
                final_decision: format!("Conditional approval with {} conditions", all_conditions.len()),
                dissenting_opinions: Vec::new(),
                resolved_at: current_timestamp_ms(),
            });
        }
        ...
        // No consensus
        Err(PlatformError::NegotiationError("No consensus reached".to_string()))
    }
    ...
    fn resolve_by_voting_static(session: &NegotiationSession) -> Result<Resolution> {
        ...
        if total_votes == 0 {
            return Err(PlatformError::NegotiationError("No valid votes".to_string()));
        }
        
        // Majority wins (conditional counts as approve)
        let approve_total = approve_count + conditional_count;
        
        let dissenting = positions.iter()
        ...
        if approve_total > reject_count {
            Ok(Resolution {
                ...
                final_decision: format!("Approved by majority vote ({}/{})", approve_total, total_votes),
            ...
            Ok(Resolution {
                ...
                final_decision: format!("Rejected by majority vote ({}/{})", reject_count, total_votes),
                dissenting_opinions: dissenting,
                resolved_at: current_timestamp_ms(),
            })
        } else {
    ...
    fn calculate_consensus_level_static(positions: &[&Position]) -> f64 {
        ...
        let approve_count = positions.iter()
            .filter(|p| matches!(p.stance, Stance::Approve))
            .count();
        
        let conditional_count = positions.iter()
...
mod tests {
    ...
    fn create_test_topic(participants: Vec<&str>) -> NegotiationTopic {
        NegotiationTopic {
            ...
            deadline_ms: current_timestamp_ms() + 3600_000, // 1 hour from now
        }
    }
    ...
    fn test_voting_resolution() {
        ...
        protocol.submit_position(
            ...
            Position {
                ...
                stance: Stance::Reject { reason: "Not ready".to_string() },
                arguments: vec![],
                evidence: vec![],
            },
        ).unwrap();
    ...
    fn test_escalation_resolution() {
        ...
        protocol.submit_position(
            ...
            Position {
                agent_id: "agent2".to_string(),
                stance: Stance::Reject { reason: "Disagree".to_string() },
                arguments: vec![],
                evidence: vec![],
            },
        ).unwrap();
    ...
    fn test_consensus_level_calculation() {
        let positions = vec![
            ...
            Position {
                agent_id: "agent3".to_string(),
                stance: Stance::Reject { reason: "No".to_string() },
                arguments: vec![],
                evidence: vec![],
            },
        ];
