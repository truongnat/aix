...
impl SharedMemory {
    ...
    pub fn rollback(&mut self, key: &str, version: u64) -> Result<()> {
        ...
        let value = history.iter()
            .find(|v| v.version == version)
            .ok_or_else(|| PlatformError::NotFound(format!("Version {} not found for key: {}", version, key)))?
            .clone();
...
impl SharedMemoryTrait for SharedMemory {
    ...
    fn write(&mut self, key: &str, value: MemoryValue, agent_id: &str) -> Result<()> {
        ...
        // Add to history
        self.history.entry(key.to_string())
    ...
    fn merge(&mut self) -> Result<Vec<MergeConflict>> {
        ...
        for write in self.pending_writes.drain(..) {
            writes_by_key.entry(write.key.clone())
                .or_insert_with(Vec::new)
                .push(write);
        }
...
impl MemoryValue {
    ...
    fn validate(&self) -> Result<()> {
        match &self.crdt_metadata {
            CRDTMetadata::LWWRegister { timestamp, agent_id } => {
                if agent_id.is_empty() {
                    return Err(PlatformError::InvalidInput("Agent ID cannot be empty".to_string()));
                }
                if *timestamp == 0 {
                    return Err(PlatformError::InvalidInput("Timestamp cannot be zero".to_string()));
                }
            }
            CRDTMetadata::GCounter { counts } => {
                if counts.is_empty() {
                    return Err(PlatformError::InvalidInput("G-Counter counts cannot be empty".to_string()));
                }
            }
            CRDTMetadata::ORSet { adds, removes } => {
                if adds.is_empty() && removes.is_empty() {
                    return Err(PlatformError::InvalidInput("OR-Set must have adds or removes".to_string()));
                }
            }
        }
        Ok(())
...
fn merge_crdt_values(local: &MemoryValue, remote: &MemoryValue) -> Result<MemoryValue> {
    match (&local.crdt_metadata, &remote.crdt_metadata) {
        ...
        (CRDTMetadata::GCounter { counts: c1 },
         CRDTMetadata::GCounter { counts: c2 }) => {
            let mut merged_counts = c1.clone();
            for (agent, count) in c2 {
                merged_counts.entry(agent.clone())
            ...
            Ok(MemoryValue {
                ...
                crdt_metadata: CRDTMetadata::GCounter { counts: merged_counts },
            })
        }
        
        // OR-Set: Union of adds, union of removes
        (CRDTMetadata::ORSet { adds: a1, removes: r1 },
         CRDTMetadata::ORSet { adds: a2, removes: r2 }) => {
            ...
            let elements: Vec<String> = merged_adds.iter()
                .filter(|(elem, uid)| {
                    !merged_removes.iter().any(|(r_elem, r_uid)| r_elem == elem && r_uid == uid)
                })
