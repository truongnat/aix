...
impl CausalTracer {
    ...
    pub fn query_causal_chain(&self, output_id: &str) -> Result<Vec<CausalNode>> {
        ...
        let output_node = graph.nodes.iter()
            ...
            .ok_or_else(|| crate::platform::PlatformError::NotFound(
...
fn is_acyclic(graph: &CausalGraph) -> bool {
    ...
    for node in &graph.nodes {
        adj_list.entry(node.node_id.clone()).or_insert_with(Vec::new);
    }
    for edge in &graph.edges {
    ...
    for node in graph.nodes.iter() {
        if !visited.contains(&node.node_id) {
            if has_cycle(&node.node_id, &adj_list, &mut visited, &mut in_path) {
                return false;
            }
        }
    }
...
mod tests {
    ...
    fn test_log_step_trigger() {
        ...
        tracer.log_step_trigger(&"step1".to_string(), cause).unwrap();
    ...
    fn test_query_causal_chain() {
        ...
        tracer.log_step_trigger(
            &"step1".to_string(),
            TriggerCause::UserRequest { request_id: "req1".to_string() },
        ).unwrap();
    ...
    fn test_dependency_chain() {
        ...
        tracer.log_step_trigger(
            &"step2".to_string(),
            TriggerCause::DependencyResolved { dependency_step: "step1".to_string() },
        ).unwrap();
        
        // Step 3 triggered by step 2 completion
        tracer.log_step_trigger(
            &"step3".to_string(),
            TriggerCause::DependencyResolved { dependency_step: "step2".to_string() },
        ).unwrap();
