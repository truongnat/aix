...
impl AdaptivePlanner {
    ...
    pub fn generate_plan(
        ...
        let estimated_duration_ms = planned_steps.iter().map(|s| s.estimated_duration_ms).sum();

        // Calculate overall confidence score
        let confidence_score = if planned_steps.is_empty() {
            1.0
        } else {
            planned_steps.iter().map(|s| s.confidence).sum::<f64>() / planned_steps.len() as f64
        };
    ...
    pub fn replan(
        ...
        if current_plan.plan_version >= self.config.max_replans {
            return Err(PlatformError::ReplanFailed(format!(
        ...
        let best = scored
            ...
            .ok_or_else(|| {
                PlatformError::ReplanFailed("No viable alternative found".to_string())
            })?;
    ...
    pub fn apply_feedback_recommendations(
        ...
        for recommendation in recommendations {
            ...
            match &recommendation.action {
                RecommendedAction::AddRetry {
                    step_id,
                    max_retries: _,
                } => {
                    // Find the step and add retry metadata
                    if let Some(step) = updated_plan
                        .steps
                        .iter_mut()
                        .find(|s| &s.step_id == step_id)
                    {
                        ...
                        step.estimated_duration_ms =
                            (step.estimated_duration_ms as f64 * 1.2) as u64;
                    }
                }
                RecommendedAction::AddValidation {
                    step_id,
                    validation_type,
                } => {
                    // Add a validation step after the target step
                    if let Some(pos) = updated_plan
                        .steps
                        .iter()
                        .position(|s| &s.step_id == step_id)
                    {
                        ...
                        updated_plan
                            .dependencies
                            .insert(validation_step.step_id.clone(), vec![step_id.clone()]);
                    }
                }
        ...
        updated_plan.estimated_duration_ms = updated_plan
            .steps
            .iter()
        ...
        if !updated_plan.steps.is_empty() {
            updated_plan.confidence_score =
                updated_plan.steps.iter().map(|s| s.confidence).sum::<f64>()
                    / updated_plan.steps.len() as f64;
        }
    ...
    fn validate_dependencies(
        ...
        for (step_id, deps) in dependencies {
            for dep in deps {
                if !step_ids.contains(dep.as_str()) {
                    return Err(PlatformError::PlanningError(format!(
        ...
        for step in steps {
            if !visited.contains(&step.step_id)
                && self.has_cycle(&step.step_id, dependencies, &mut visited, &mut rec_stack)
            {
    ...
    fn generate_alternatives(
        ...
        match trigger {
            ReplanTrigger::TestFailure { step_id, .. } => {
                ...
                if let Some(pos) = alt1.steps.iter().position(|s| &s.step_id == step_id) {
                    ...
                    // Update dependencies
                    alt1.dependencies
                        .insert(debug_step.step_id.clone(), vec![step_id.clone()]);
                    alt1.dependencies
                        .insert(fix_step.step_id.clone(), vec![debug_step.step_id.clone()]);
                }
            }
            ReplanTrigger::BlockedDependency { step_id, .. } => {
                // Add workaround step
        ...
        if !alt1.steps.is_empty() {
            alt1.confidence_score =
                alt1.steps.iter().map(|s| s.confidence).sum::<f64>() / alt1.steps.len() as f64;
        }
        ...
        let remaining_steps: Vec<_> = current_plan
            .steps
            .iter()
        ...
        if !remaining_steps.is_empty() && remaining_steps.len() < current_plan.steps.len() {
            ...
            alt2.confidence_score =
                alt2.steps.iter().map(|s| s.confidence).sum::<f64>() / alt2.steps.len() as f64;
    ...
    fn score_alternatives(
        ...
        for (idx, plan) in alternatives.into_iter().enumerate() {
            ...
            let mut score =
                confidence_weight * normalized_confidence + duration_weight * normalized_duration;
    ...
    fn merge_plans(
        ...
        for completed_step in completed {
            if !merged.steps.iter().any(|s| &s.step_id == completed_step) {
                return Err(PlatformError::ReplanFailed(format!(
                    "Completed step {} not preserved in new plan",
                    completed_step
                )));
            }
        }
...
mod tests {
    ...
    fn test_generate_plan() {
        ...
        assert_eq!(
            plan.dependencies.get("step2").unwrap(),
            &vec!["step1".to_string()]
        );
    }
    ...
    fn test_replan_adds_remediation_steps() {
        ...
        assert!(
            new_plan.steps.len() > original_len,
    ...
    fn test_apply_feedback_recommendations() {
        ...
        let updated_plan = planner
            .apply_feedback_recommendations(&plan, &recommendations)
            .unwrap();
    ...
    fn test_feedback_recommendations_respect_confidence_threshold() {
        ...
        let low_confidence_rec = vec![Recommendation {
            recommendation_id: "rec1".to_string(),
            workflow_id: "wf1".to_string(),
            action: RecommendedAction::AddValidation {
                step_id: "step1".to_string(),
        ...
        let updated_plan = planner
            .apply_feedback_recommendations(&plan, &low_confidence_rec)
            .unwrap();
