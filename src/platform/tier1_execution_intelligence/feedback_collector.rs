...
impl FeedbackCollector {
    ...
    pub fn aggregate_signals(&self, time_window: Duration) -> Result<FeedbackSummary> {
        ...
        let avg_performance_metrics: HashMap<String, f64> = performance_metrics
            .iter()
    ...
    pub fn get_recommendations(&self, workflow_id: &str) -> Result<Vec<Recommendation>> {
        let summary =
            self.aggregate_signals(Duration::from_secs(self.config.aggregation_window_secs))?;
    ...
    pub fn update_recommendation_effectiveness(
        ...
        if let Some(history) = self.recommendation_history.get_mut(workflow_id) {
            if let Some(effectiveness) = history
                .iter_mut()
                .find(|e| e.recommendation_id == recommendation_id)
            {
                ...
                if before.incident_count > 0 {
                    let incident_improvement = (before.incident_count as f64
                        - after.incident_count as f64)
    ...
    pub fn get_effectiveness_history(
        &self,
        workflow_id: &str,
    ) -> Vec<&RecommendationEffectiveness> {
        self.recommendation_history
            .get(workflow_id)
...
mod tests {
    ...
    fn test_aggregate_signals() {
        ...
        for i in 0..5 {
            collector
                .ingest_signal(ProductionSignal {
        ...
        let summary = collector
            .aggregate_signals(Duration::from_secs(3600))
            .unwrap();
    ...
    fn test_update_recommendation_effectiveness() {
        ...
        collector
            .update_recommendation_effectiveness("rec1", "wf1", after_metrics)
            .unwrap();
        ...
        assert!(
            score > 0.0,
            "Expected positive improvement score, got {}",
            score
        );
    }
    ...
    fn test_multiple_recommendation_types() {
        ...
        let has_retry = recommendations
            .iter()
            .any(|r| matches!(r.action, RecommendedAction::AddRetry { .. }));
        let has_validation = recommendations
            .iter()
    ...
    fn test_performance_metric_recommendations() {
        ...
        let has_optimization = recommendations
            .iter()
            .any(|r| matches!(r.action, RecommendedAction::OptimizeResource { .. }));

        assert!(
            has_optimization,
            "Should recommend resource optimization for high latency"
        );
    }
}
