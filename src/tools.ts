...
const INDEX_PROJECT_DEFAULT_INCLUDE =
  '**/*.md,**/*.ts,**/*.tsx,**/*.js,**/*.jsx,**/*.mjs,**/*.cjs,**/*.py,**/*.go,**/*.java,**/*.rs,**/*.json,**/*.yaml,**/*.yml,**/*.toml';
...
async function cmdIndexProject(args: ParsedArgs) {
  ...
  for (const pattern of includes) {
    ...
    for (const f of matches) fileSet.add(f);
  }
  ...
  if (dry) {
    console.log(
      `Would index ${files.length} files -> ${manifest.length} chunks (skipped binary=${skippedBinary}, skipped large=${skippedLarge})`,
    );
    return;
  }
...
function normalizeWikiMdToHtmlHref(url: string): string {
  const u = url.trim();
...
function safeWikiHref(url: string): string | null {
  const u = url.trim();
...
function parseMdLinkWiki(line: string, start: number): { end: number; html: string } | null {
...
function inlineMdToHtmlWikiNoLinks(line: string): string {
  let idx = 0;
  let out = '';
...
function inlineMdToHtmlWiki(line: string): string {
  let idx = 0;
  let out = '';
  while (idx < line.length) {
    const tick = line.indexOf('`', idx);
    ...
    const candidates: { pos: number; kind: 'tick' | 'star' | 'bracket' }[] = [];
    if (tick >= 0) candidates.push({ pos: tick, kind: 'tick' });
    if (star >= 0) candidates.push({ pos: star, kind: 'star' });
    if (bracket >= 0) candidates.push({ pos: bracket, kind: 'bracket' });
...
function tryParseMarkdownTable(lines: string[], start: number): { html: string; next: number } | null {
  const row0 = lines[start];
  if (!row0?.includes('|')) return null;
  const head = splitMarkdownTableRow(row0);
  if (head.length < 2) return null;
  const sepLine = lines[start + 1];
  if (!sepLine || !isMarkdownTableSeparatorRow(sepLine)) return null;
  const body: string[][] = [];
  let j = start + 2;
...
function mdBodyToHtmlWiki(md: string): string {
  const lines = md.replace(/\r\n/g, '\n').split('\n');
  const blocks: string[] = [];
  let i = 0;
  let inCode = false;
  const codeBuf: string[] = [];
  const flushPara = (para: string[]) => {
...
function wikiRelUrl(outDir: string, fromHtmlRel: string, toHtmlRel: string): string {
  const fromAbs = join(outDir, fromHtmlRel);
  const toAbs = join(outDir, toHtmlRel);
  let r = relative(dirname(fromAbs), toAbs).replace(/\\/g, '/');
  if (r === '') r = basename(toHtmlRel);
  return r;
}

function wikiNavHtml(outDir: string, currentHtmlRel: string, mdRelPaths: string[]): string {
  const toHtml = (mdRel: string) => mdRel.replace(/\.md$/i, '.html');
  const links: string[] = [];
  const homeTarget = 'index.html';
  const homeCls = currentHtmlRel === homeTarget ? ' class="active"' : '';
  links.push(
    `<a href="${escapeHtmlWiki(wikiRelUrl(outDir, currentHtmlRel, homeTarget))}"${homeCls}>Home</a>`,
  );
  for (const rel of mdRelPaths) {
    const target = toHtml(rel);
    const cls = currentHtmlRel === target ? ' class="active"' : '';
    links.push(
      `<a href="${escapeHtmlWiki(wikiRelUrl(outDir, currentHtmlRel, target))}"${cls}>${escapeHtmlWiki(rel.replace(/\.md$/i, ''))}</a>`,
    );
  }
...
function wikiWrapPage(title: string, navInner: string, bodyInner: string, footerScript: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
type WikiSearchEntry = { target: string; title: string; text: string };

function wikiSearchExcerpt(markdown: string): string {
...
function wikiFooterSearchScript(outDir: string, currentHtmlRel: string, entries: WikiSearchEntry[]): string {
...
function openHtmlFileInBrowser(filePath: string): void {
  const abs = resolve(filePath);
  if (!existsSync(abs)) return;
  try {
    if (process.platform === 'darwin') execFileSync('open', [abs], { stdio: 'ignore' });
    else if (process.platform === 'win32') execFileSync('cmd', ['/c', 'start', '', abs], { stdio: 'ignore' });
    else execFileSync('xdg-open', [abs], { stdio: 'ignore' });
  } catch {
    console.warn(`Could not open browser for ${abs}`);
  }
}

function runGenerateWiki(docsDir: string, outDir: string): { mdCount: number } {
  if (!existsSync(docsDir)) throw new Error(`Missing docs dir: ${docsDir}`);
  const mdFiles = globSync('**/*.md', { cwd: docsDir, nodir: true }).sort();
  if (mdFiles.length === 0) throw new Error(`No .md files under ${docsDir}`);

  const toHtmlRel = (mdRel: string) => mdRel.replace(/\.md$/i, '.html');
  const homeMd =
    mdFiles.includes('project-overview.md') ? 'project-overview.md' : mdFiles.includes('tree.md') ? 'tree.md' : mdFiles[0];

  const searchEntries: WikiSearchEntry[] = mdFiles.map((rel) => {
  ...
  const homeTitle =
    typeof homeParsed.data.title === 'string' && homeParsed.data.title.trim()
...
function extractSkillTriggers(info: ReturnType<typeof readSkillInfo>): string[] {
...
function loadRoutingSkillRows(repoRoot: string, includeTemplate = false): RoutingSkillRow[] {
...
function promptContainsTriggerTokensInOrder(promptTokens: string[], triggerTokens: string[]): boolean {
...
type RoutingTriggerMatch = {
  trigger: string;
  normalized: string;
  score: number;
};

function matchPromptToTriggers(prompt: string, triggers: string[]): RoutingTriggerMatch[] {
  ...
  return triggers
    .map((trigger) => {
      ...
      if (!isExact && !isOrderedTokenMatch) return null;
      return {
        trigger,
        normalized,
...
type RoutingEvalCase = {
  id?: string;
  prompt: string;
  /** Primary label for reports; optional when `expected_any_of` is set (first any_of used as primary). */
  expected_skill?: string;
  /** If set (non-empty), pass when the top predicted skill is any of these (overlap-tolerant routing). */
  expected_any_of?: string[];
  /** If the top predicted skill is one of these, fail (known-wrong neighbors for overlap / negative tests). */
  hard_negative_skills?: string[];
  note?: string;
};

function routingCaseAllowedSkills(testCase: RoutingEvalCase): string[] {
...
function routingCasePrimaryExpected(testCase: RoutingEvalCase): string {
  if (typeof testCase.expected_skill === 'string' && testCase.expected_skill.length > 0) {
    return testCase.expected_skill;
  }
  const any = testCase.expected_any_of;
  if (Array.isArray(any) && any.length > 0) return any[0]!;
  return '';
}
...
function cmdEvalSkillRouting(args: ParsedArgs, repoRoot: string) {
  ...
  const rawCases = JSON.parse(readFileSync(datasetPath, 'utf8')) as RoutingEvalCase[];
  ...
  if (asMarkdown) {
    ...
    rows.forEach((row) => {
      ...
      const exp =
        row.expectedAnyOf.length > 1 ? `\`${row.expectedAnyOf.join('`, `')}\` (any)` : `\`${row.expectedSkill}\``;
  ...
  if (failures.length > 0) {
    ...
    failures.forEach((row) => {
      ...
      const reason =
        row.status === 'missing_triggers' ? `expected skill has 0 extracted triggers` :
      ...
      const want =
        row.expectedAnyOf.length > 1 ? `one of: ${row.expectedAnyOf.join(', ')}` : row.expectedSkill;
...
function findSectionLabelPosition(response: string, label: string): number {
  const pattern = new RegExp(
    `(?:^|\\n)\\s*(?:#{1,6}\\s*)?(?:\\d+\\.\\s+)?(?:\\*\\*)?${escapeRegExp(label)}(?:\\*\\*)?\\s*(?:—|:|$)`,
    'im',
  );
...
type OutputEvalHeuristicResult = {
  pass: boolean;
...
function evalOutputFormatCaseHeuristic(response: string, spec: OutputEvalSpec): OutputEvalHeuristicResult {
...
function parseJsonObjectFromJudgeText(raw: string): OutputJudgeLlmJson | null {
  const trimmed = raw.trim();
  try {
    return JSON.parse(trimmed) as OutputJudgeLlmJson;
  } catch {
    const start = trimmed.indexOf('{');
    const end = trimmed.lastIndexOf('}');
    if (start >= 0 && end > start) {
      try {
        return JSON.parse(trimmed.slice(start, end + 1)) as OutputJudgeLlmJson;
      } catch {
        return null;
      }
    }
...
async function judgeOutputFormatWithLlm(params: {
  ...
  response: string;
  spec: OutputEvalSpec;
}): Promise<{ ok: true; judge: OutputJudgeLlmJson } | { ok: false; error: string }> {
...
async function cmdEvalSkillOutputFormat(args: ParsedArgs, repoRoot: string) {
  ...
  if (!existsSync(specPath)) throw new Error(`Missing output eval spec file: ${specPath}`);
  if (!existsSync(casesPath)) throw new Error(`Missing output eval cases file: ${casesPath}`);

  const specs = JSON.parse(readFileSync(specPath, 'utf8')) as OutputEvalSpec[];
  const cases = JSON.parse(readFileSync(casesPath, 'utf8')) as OutputEvalCase[];

  if (!Array.isArray(specs) || specs.length === 0) throw new Error(`Output eval specs must be a non-empty JSON array: ${specPath}`);
  if (!Array.isArray(cases) || cases.length === 0) throw new Error(`Output eval cases must be a non-empty JSON array: ${casesPath}`);
  ...
  type OutputEvalRow = {
    ...
    orderOk: boolean;
    codeFenceOk: boolean;
    requireCodeFence: boolean;
    heuristicPass: boolean;
    judgeRationale?: string | null;
    judgeError?: string | null;
    judgeMissingSections?: string[];
    judgeOrderOk?: boolean;
    judgeCodeFenceOk?: boolean;
  };
  ...
  if (asMarkdown) {
    ...
    if (mode === 'ai-judge') {
      ...
      rows.forEach((row) => {
        ...
        console.log(
          `| \`${row.id}\` | \`${row.skill}\` | ${row.pass ? 'yes' : 'no'} | ${row.heuristicPass ? 'yes' : 'no'} | ${jm} | ${row.judgeOrderOk === false ? 'bad' : 'ok'} | ${row.requireCodeFence ? (row.judgeCodeFenceOk ? 'ok' : 'bad') : 'n/a'} | ${rat} |`,
        );
      });
    } else {
  ...
  if (failures.length > 0) {
    ...
    failures.forEach((row) => {
      ...
      console.log(`        ${chalk.dim(row.prompt)}`);
      if (row.judgeError) console.log(`        judge: ${chalk.red(row.judgeError)}`);
      if (mode === 'ai-judge' && row.judgeRationale) console.log(`        rationale: ${chalk.dim(row.judgeRationale)}`);
      if (row.missingSections.length > 0) console.log(`        heuristic missing: ${chalk.dim(row.missingSections.join(', '))}`);
      if (!row.orderOk) console.log(`        heuristic order: ${chalk.dim('section labels out of order')}`);
      if (row.requireCodeFence && !row.codeFenceOk) console.log(`        heuristic code: ${chalk.dim('required code fence missing')}`);
    });
  }
...
async function cmdBuildSkillIndex(args: ParsedArgs, repoRoot: string) {
  ...
  for (const dir of dirs) {
    ...
    const meta = (matter(raw).data.metadata ?? {}) as Record<string, string>;
...
function cmdAnalyzeSkills(args: ParsedArgs, repoRoot: string) {
  ...
  if (json) { console.log(JSON.stringify(outRows, null, 2)); return; }

  const tierBadge = (t: string) =>
    t === 'strong'  ? chalk.green('strong ') :
...
function resolveConsumerBundleDir(projectDir: string): { bundleDir: string; kind: 'devkit' | 'legacy' } | null {
  const devkit = join(projectDir, '.agents', 'devkit');
  if (existsSync(join(devkit, '.devkit-bundle'))) return { bundleDir: devkit, kind: 'devkit' };
  const legacy = join(projectDir, 'vendor', 'own-skills');
  if (existsSync(join(legacy, '.own-skills-bundle'))) return { bundleDir: legacy, kind: 'legacy' };
  return null;
}

function symlinkResolvesUnder(linkPath: string, expectedDir: string): boolean {
  try {
    const st = lstatSync(linkPath);
    if (!st.isSymbolicLink()) return false;
    const target = readlinkSync(linkPath, { encoding: 'utf8' }) as string;
...
function strictBundleInstallWarnings(projectDir: string, bundleDir: string): string[] {
  ...
  for (const [ideRoot, sub] of skillBases) {
    ...
    for (const name of readdirSync(root)) {
      if (name === '.install-manifest') continue;
...
function cmdVerifyBundleInstall(args: ParsedArgs, _repoRoot: string) {
  ...
  if (!resolved) {
    errs.push(
      'Missing installed bundle: expected .agents/devkit/.devkit-bundle or legacy vendor/own-skills/.own-skills-bundle',
    );
  }
  const bundleDir = resolved?.bundleDir ?? join(projectDir, '.agents', 'devkit');
  if (resolved && !existsSync(join(bundleDir, 'scripts'))) errs.push(`Missing ${join(bundleDir, 'scripts')}`);
  ...
  if (resolved && !existsSync(toolsJs)) errs.push(`Missing ${toolsJs}`);
  const canValidateSkills =
    resolved &&
    existsSync(toolsJs) &&
...
function cmdQueryGraph(args: ParsedArgs) {
  ...
  let results: { id: string; name: string; type: string; file: string; line: number }[] = [];
...
function cmdGenerateGapAnalysis(args: ParsedArgs, repoRoot: string) {
  ...
  const gapsByDomain: { domain: string; expected: string[]; missing: string[] }[] = [];
  ...
  for (const domain of taxonomyDomains) {
    ...
    if (missing.length > 0) gapsByDomain.push({ domain, expected, missing });
  }
  ...
  if (gapsByDomain.length === 0) {
    ...
    for (const { domain, missing } of gapsByDomain) {
      lines.push(`### \`${domain}\``, ``);
      for (const m of missing) lines.push(`- [ ] \`${m}\` — not found`);
      lines.push(``);
    }
  }
  ...
  if (untaxonomized.length === 0) {
    ...
    lines.push(``);
    for (const s of untaxonomized.sort()) lines.push(`- \`${s}\` — add to DOMAIN_TAXONOMY in tools.ts`);
  }
  ...
  if (noRefs.length === 0) {
    ...
    lines.push(``);
    for (const s of noRefs.sort()) lines.push(`- \`${s}\``);
  }

  lines.push(``, `---`, ``, `## Suggested improvements`);
  const suggestions: string[] = [];
  if (gapsByDomain.length > 0) suggestions.push(`Add ${gapsByDomain.reduce((n, g) => n + g.missing.length, 0)} missing skills from taxonomy gaps above.`);
  if (untaxonomized.length > 0) suggestions.push(`Register ${untaxonomized.length} untaxonomized skill(s) in DOMAIN_TAXONOMY in \`src/tools.ts\`.`);
  if (noRefs.length > 0) suggestions.push(`Add \`references/\` folders to ${noRefs.length} skill(s) without one.`);
  if (suggestions.length === 0) suggestions.push(`No gaps detected — re-run quarterly or after adding 5+ skills.`);
  lines.push(``);
  ...
  if (untaxonomized.length > 0) {
    ...
    for (const s of untaxonomized) console.log(`    ${chalk.dim(s)}`);
  }
  if (noRefs.length > 0) {
    console.log(chalk.yellow(`\n  Missing references/ (${noRefs.length}):`));
    for (const s of noRefs) console.log(`    ${chalk.dim(s)}`);
  }
}
...
${chalk.bold('BUNDLE')}
  ...
  ${chalk.yellow('analyze-doc')}            Convert document files to Markdown  ${chalk.dim('<file...>')}
...
async function main() {
  ...
  switch (normalizedCmd) {
    ...
    case 'verify-bundle-install':
      ...
      break;
    case 'analyze-doc':
      analyzeDocuments((args._ as string[]).slice(1));
      break;
    case 'build-kb':
