...
name: bun-cli-pro
description: >
  Professional Bun CLI development: Commander.js subcommands, config
  persistence, API clients, UX polish with ora spinners and chalk output, and
  binary compilation.


  Use this skill when building CLI tools, designing command hierarchies,
  managing user configuration, parsing arguments, making HTTP requests from CLI,
  and compiling executables.


  Combine with **`api-design-pro`** for REST API patterns the CLI calls,
  **`nestjs-neo4j-pro`** for graph-backed CLIs.


  Triggers: "CLI", "commander", "bun build", "binary", "configuration", "conf
  library", "ora spinner", "chalk", "command line", "argument parsing"
x-kind: domain
x-version: 0.1.0
x-roles: []
x-tags: []
