...
name: to-issues-pro
description: >
  Break a plan, PRD, or GitHub issue into independently-implementable GitHub
  issues using vertical tracer-bullet slices. Each issue cuts end-to-end through
  all layers (UI → API → DB) rather than horizontal layer-by-layer
  decomposition. Classifies issues as HITL (requires human review) or AFK (can
  be implemented autonomously). Use when the user has a spec or PRD and wants to
  turn it into an actionable issue backlog.


  Triggers: "break this into issues", "create GitHub issues", "to-issues",
  "slice this into tickets", "make issues from the PRD", "vertical slices",
  "tracer bullets".


  Combine with **`to-prd-pro`** to produce the PRD first, and **`planning-pro`**
  for dependency ordering and milestone sequencing.
x-kind: domain
x-version: 0.1.0
x-roles: []
x-tags: []
