...
description: >
  Professional skill for reverse-engineering UI from images, screenshots,
  mockups, or reference UIs into clean, maintainable, system-based code without
  blind pixel cloning.


  Use when rebuilding UI from a screenshot or mockup without a proper design
  file, extracting layout structure from a reference image, or translating a
  visual into maintainable system-based code.


  Triggers: "image to code", "screenshot to UI", "mockup implementation",
  "reverse engineer UI", "clone UI", "design from reference", "visual to code",
  "layout extraction", "reference UI", "screenshot rebuild".
x-kind: domain
x-version: 0.1.0
x-roles: []
x-tags: []
