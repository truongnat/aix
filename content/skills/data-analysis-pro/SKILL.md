...
description: >-
  Production-grade tabular data analysis in Python: analytical pipeline and
  grain model (row unit, joins, lineage), EDA and cleaning discipline,
  descriptive statistics and honest visualization, pivot-style workflows,
  Parquet/CSV/SQLite IO, failure modes (join explosions, timezone bugs,
  lookahead leakage, Simpson’s paradox, p-hacking, Excel precision), decision
  trade-offs (pandas vs SQL pushdown, notebook vs pipeline, descriptive vs
  causal claims), reproducibility and anti-hallucination guardrails for numbers,
  spreadsheet delivery (openpyxl charts, validation, freeze panes), and
  integration with SQL, business analysis, and security for PII-safe exports.


  Use when analyzing CSV/Parquet/SQLite exports, profiling datasets, building
  aggregates and charts, or authoring Excel analytics — not for unstructured
  PDF/image semantic reading (content-analysis-pro).


  Use with content-analysis-pro, business-analysis-pro, postgresql-pro,
  sql-data-access-pro, security-pro, testing-pro as needed.
