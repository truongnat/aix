...
import { createKbVectorStore } from './lib/vector-store.js';
function readText(path) {
    return readFileSync(path, 'utf8');
}
function analyzeDocuments(filePaths) {
...
async function cmdIndexProject(args) {
 ...
 */
function loadSkillEmbeddings(repoRoot) {
 ...
 */
function semanticRoutingFallback(prompt, skillEmbeddings, threshold = 0.5) {
...
class _WorkerPool {
    ...
    taskQueue = new Map();
    taskId = 0;
    constructor(workerScript, poolSize = Math.max(2, Math.floor(os.cpus().length / 2))) {
...
async function cmdBuildSkillIndex(args, repoRoot) {
...
async function buildKb(repoRoot, dry = false) {
    const cfg = loadKbConfig(repoRoot);
...
async function queryKb(repoRoot, q, topK, indexDir) {
...
async function cmdQueryKb(args, repoRoot) {
...
async function cmdQueryKbBatch(args, repoRoot) {
    const topK = Number(args.k || args['top-k'] || 5);
    const queries = [];
...
function cmdVerifyKb(_args, repoRoot) {
    ...
    if (!existsSync(indexPath))
        errors.push(`Missing embeddings index: ${indexPath}`);
    ...
    if (errors.length === 0) {
        ...
        if (!Array.isArray(m))
            errors.push('Manifest format invalid');
        if (store.size() !== m.length)
            errors.push(`Vector count (${store.size()}) != manifest count (${m.length})`);
    }
    if (errors.length > 0) {
...
async function cmdBuildGraph(args) {
    const targetDir = resolve(String(args.dir || args.d || '.'));
...
function cmdSyncCatalogs(args, repoRoot) {
    const dirs = listSkillDirs(repoRoot, false);
    const dry = Boolean(args['dry-run']);
    ...
    if (existsSync(skillsReadmePath))
        skillsReadmeContent = readFileSync(skillsReadmePath, 'utf8');
    if (existsSync(skillsLayoutPath))
        skillsLayoutContent = readFileSync(skillsLayoutPath, 'utf8');
    ...
    const newLayoutContent = skillsLayoutContent.replace(/(skills\/\n)([\s\S]*?)(\n```)/, `$1${newLayoutTree}$3`);
    ...
    if (nonConforming.length > 0) {
        ...
        for (const e of nonConforming)
            console.log(`    ${chalk.red(e.folder)}`);
    }
    if (missingRefs.length > 0) {
        console.log(chalk.yellow(`\n  Missing references/ (${missingRefs.length}):`));
        for (const e of missingRefs)
            console.log(`    ${chalk.dim(e.folder)}`);
    }
...
async function main() {
    ...
    switch (normalizedCmd) {
        ...
        case 'verify-bundle-install':
            ...
            break;
        case 'analyze-doc':
            analyzeDocuments(args._.slice(1));
            break;
        case 'build-kb':
