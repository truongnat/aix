...
async function transformerEmbed(text) {
    ...
    return Array.from(output.data).slice(0, MODEL_DIMS);
}
...
function loadGraphForEnrichment(graphPath) {
    if (cachedGraphPath === graphPath && cachedGraph)
        return cachedGraph;
    if (!existsSync(graphPath))
        return null;
    try {
        cachedGraph = JSON.parse(readFileSync(graphPath, 'utf8'));
...
export function enrichWithGraphContext(text, graphPath) {
 ...
 */
export async function embedTextAsync(text, options = {}) {
...
export async function embedBatch(texts, options = {}) {
    ...
    if (uncachedTexts.length > 0) {
        ...
        for (let i = 0; i < uncachedIndices.length; i++) {
            ...
            embeddingCache.set(cacheKey, embedding);
            if (onProgress)
                onProgress(i + 1, uncachedTexts.length);
        }
    }
    else {
        if (onProgress)
 ...
 */
export function defaultGraphPath(baseDir) {
