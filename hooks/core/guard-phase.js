...
function planApproved(sessionDir, stateContent) {
...
function hasImplementationEvidence(sessionDir) {
...
function verifyReady(sessionDir) {
...
function guardPhase(options) {
  ...
  if (command === "harness-run") {
    ...
    if (!plan.ok) {
      return { ok: false, status: "blocked", command, ...plan };
    }
  }
  ...
  if (command === "harness-ship") {
    ...
    if (!verify.ok) {
      return { ok: false, status: "blocked", command, ...verify };
    }
  }
...
function main() {
  try {
    ...
    if (options.help) {
      printHelp("guard-phase.js", [
        "Usage:",
        "  node hooks/core/guard-phase.js --command harness-run --session .harness/sessions/<id> [--json]",
        "",
        "Checks:",
