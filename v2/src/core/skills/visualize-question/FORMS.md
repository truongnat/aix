...
How to build the call to `askVisualQuestion` (from `tools/preview`) so it satisfies the `Output Contract` in `SKILL.md`. You generate the HTML; this guide covers the inputs and the contract you must honor.
...
The page body you generate, following `tools/preview/references/design-guide.md`:
